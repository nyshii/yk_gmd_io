...
from ...materials import YAKUZA_SHADER_NODE_GROUPS, PATTERN_SHADERS, YAKUZA_GMDMATERIAL_NODEGROUP
...
class BaseGMDSceneGatherer(abc.ABC):
    ...
    def blender_material_to_gmd_attribute_set(self, material: bpy.types.Material,
        ...
        elif skinned and engine < 4:
            yakuza_shader = YAKUZA_SHADER_NODE_GROUPS.get('OLD_SKINNED_SHADER')
        elif not skinned and engine >= 4: 
            yakuza_shader = YAKUZA_SHADER_NODE_GROUPS.get('DRAGON_UNSKINNED_SHADER')
        elif not skinned and engine < 4: 
            yakuza_shader = YAKUZA_SHADER_NODE_GROUPS.get('OLD_UNSKINNED_SHADER')  
        gmdmaterial_node =  YAKUZA_GMDMATERIAL_NODEGROUP
        ...
        gmdmaterial_nodes =   [node for node in material.node_tree.nodes if
                               node.bl_idname == "ShaderNodeGroup" and node.node_tree.name == gmdmaterial_node]
