...
from ...materials import get_yakuza_shader_node_groups, get_uv_scaler_node_group, \
    get_asset_uvs_node_group, decode_shader_name, get_gmdmaterial_data_node_group, \
...
class BaseGMDSceneCreator(abc.ABC):
    ...
    def make_material(self, collection: bpy.types.Collection, gmd_attribute_set: GMDAttributeSet) -> bpy.types.Material:
        ...
        def make_yakuza_node_group(node_tree: bpy.types.NodeTree):
            ...
            node.node_tree = get_yakuza_shader_node_groups(self.error, gmd_attribute_set)
            return node
        def make_gmdmaterial_node_group(node_tree: bpy.types.NodeTree):
            node = node_tree.nodes.new("ShaderNodeGroup")
            node.node_tree = get_gmdmaterial_data_node_group(self.error)
            return node
        ...
        # Make a GMDMaterial node group, and position it
        gmdmaterial_node_group = make_gmdmaterial_node_group(material.node_tree)
        gmdmaterial_node_group.location = (-480,-700)
        gmdmaterial_node_group.width = 225
        # Hook it up to the main nodegroup
        for output in gmdmaterial_node_group.outputs:
            material.node_tree.links.new(output, yakuza_shader_node_group.inputs[output.name])
        ...
        if mat_yk_data.inited == True:
            if gmd_attribute_set.shader.assume_skinned and any([x in mat_yk_data.shader_name for x in PATTERN_SHADERS]):
                    # Skinned UVs
            ...
            elif not gmd_attribute_set.shader.assume_skinned:
                # Unskinned UVs
                uv_scaler_node = material.node_tree.nodes.new('ShaderNodeGroup')
